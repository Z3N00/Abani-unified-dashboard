...
export async function POST(request: Request, context: { params: Promise<{ token: string }> }) {
  try {
    ...
    const objectPath = `containers/${encodeURIComponent(entry.containerNumber)}/${crypto.randomUUID()}-${safeFileName(file.name)}`
    ...
    await db.from('ContainerDocActivity').insert({ id: crypto.randomUUID(), entryId: entry.id, vendorDocId: vendorId, action: kind === 'document' ? 'DOCUMENT_UPLOADED' : 'DEPARTURE_PHOTO_UPLOADED', actor: 'Overseas documentation link', details: { fileName: file.name, documentType: kind === 'document' ? documentType : null }, createdAt: now })
