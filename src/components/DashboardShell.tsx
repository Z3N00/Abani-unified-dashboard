...
type IconName = 'overview' | 'containers' | 'purchasing' | 'inventory' | 'users' | 'collapse' | 'expand'
...
export default function DashboardShell({ user, children }: { user: SessionUser; children: ReactNode }) {
  ...
  const currentPage = path === '/users' ? 'Users & permissions' : links.find((link) => link.href === path)?.label ?? 'Workspace'
